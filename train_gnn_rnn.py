...
from models.gnn_rnn_models import StaticGNNRNN
...
train_dataset=MultigraphDataset(paths,12)
paths=torch.load(test_dir+"filenames")
test_dataset=MultigraphDataset(paths,12)
train_loader=DataLoader(train_dataset,batch_size=batch_size,shuffle=True)
test_loader=DataLoader(test_dataset,batch_size=batch_size,shuffle=False)

# define model
model=StaticGNNRNN(conv_type,temporal_type,device)
...
def train():
    ...
    for data in train_loader:
        optimizer.zero_grad()
        out, label=model(data)
        loss=ce(out,label.to(device))
        loss.backward()
        loss_all+=loss.item()
        pred = out.detach().cpu().max(dim=1)[1]
...
def test():
    ...
    for data in test_loader:
        out, label=model(data)
        loss_all+=ce(out.detach().cpu(),label.long()).item()*batch_size
        pred = out.detach().cpu().max(dim=1)[1]
        out_list.append(out)
        pred_list.append(pred)
        y_list.append(label.long())
    ...
    m2=auc(fpr, tpr)
    return loss_all.item()/len(test_dataset), correct/len(test_dataset)*100, m1, m2
